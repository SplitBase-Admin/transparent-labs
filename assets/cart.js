class CartRemoveButton extends HTMLElement {
  constructor() {
    ...
    this.addEventListener('click', (event) => {
      ...
      if (this.dataset.stackId) {
        cartItems.removeStack(this.dataset.stackId, event);
      } else {
...
class CartItems extends HTMLElement {
  ...
  removeStack(stackId, event) {
    const matchingItems = [];
    this.querySelectorAll('.sb-cart-drawer__item').forEach((item) => {
      if (item.querySelector(`[data-stack-id="${stackId}"]`)) {
        matchingItems.push(item);
      }
    });
    const updates = {};
    matchingItems.forEach((item) => {
      this.enableLoading(item.dataset.index);
      updates[item.dataset.id] = 0;
    });
    ...
    fetch(`${routes.cart_update_url}`, { ...fetchConfig(), ...{ body } })
      ...
      .then((state) => {
        ...
        CartPerformance.measure('remove-stack:paint-updated-sections', () => {
          ...
          if (newProgressBar && cartDrawerWrapper) {
            ...
            if (!prevThresholdMet && currentThresholdMet) {
              newProgressBar.classList.add('sb-progress-bar--just-filled');
            }
          ...
          this.updateLiveRegions(0, parsedState.errors || window.cartStrings.stackRemoved);
        ...
        CartPerformance.measureFromEvent('remove-stack:user-action', event);
